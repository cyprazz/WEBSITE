from flask import Flask, redirect, url_for, request, render_template #for all the flask
import hashlib  # included in Python library, no need to install
import psycopg2  # for database connection
import json #for the config.json
from datetime import datetime #for the datetime for the database
...
@app.route("/kapper" , methods=["GET"])
def showKapper():
    #Show the homepage
    t_message = "Zoek naar kappers op plaats"
    return render_template("kapper.html", message=t_message)

@app.route("/kapper/search", methods=["GET"])
def showSearch():
    t_message = "Zoek naar kappers op plaats"
    t_searchUpper = request.args.get("Search", "")
    t_search = t_searchUpper.lower()

    if t_searchUpper == "":
        return render_template("kapper.html", message=t_message)
    else:
        # Get naam from PostgreSQL kapperszaken table
        s = ""
        s += "SELECT naam, plaats, postcode, foto, sterren, website FROM kapperszaken"
        s += " WHERE"
        s += " plaats = '" + t_search + "'; "
        ...
        for row in db_cursor:
            listRow = list(row)
            s += "<div class='resultaat' style= 'background-image: url(" + str(listRow[3]) + ");'>"
            s += "<img class='roundUp' src='/static/svg/round.svg'>"
            s += "<p class='stars'>"
            for i in range(int(listRow[4])):
                s += "&#9733;"
            s += "</p><br>"
            s += "<h1>" + listRow[0] + "</h1>" + "<p class='locatie'>"
            s += listRow[1] + " "
            s += listRow[2] + "</p>"
            s += "<img class='roundDown' src='/static/svg/round.svg'>"
            s += "<a href='" + str(listRow[5]) +"'>website</a>"
            s +="</div>"
            results = s
        ...
        # try:
        #     searchResults1 = db_cursor.fetchone()
        # except psycopg2.Error as e:
        #     t_message = "Database error: " + e + "/n SQL: " + s
        #     return render_template("search.html", message = t_message)

        # try:
        #     searchResults2 = db_cursor.fetchone()
        # except psycopg2.Error as e:
        #     t_message = "Database error: " + e + "/n SQL: " + s
        #     return render_template("search.html", message = t_message)

        # t_message = searchResults
...
#     #   using another field, along with t_email and t_stage,
